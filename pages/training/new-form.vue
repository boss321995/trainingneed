...
const searchPackages = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:4000/api/courses/packages/search?${params}`)
...
const searchCourses = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:4000/api/courses/courses/search?${params}`)
    const data = await response.json()
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch('http://localhost:4000/api/training-needs', {
      method: 'POST',
      headers: {
...
onMounted(async () => {
  // โหลดระดับงาน
  try {
    const response = await fetch('http://localhost:4000/api/courses/job-levels')
